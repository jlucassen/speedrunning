...
from inspect_ai.model import GenerateConfig, get_model, ChatMessageAssistant
...
@solver
def bf_solver(force_len, continue_str, stop_str):
    async def solve(state: TaskState, generate: Generate):
        model = get_model()
        response = await model.generate(state.input_text).completion
        total_len = len(response.completion)
        while total_len < force_len:
            response = await model.generate(state.input_text).completion
            total_len = len(response.completion)

        print(response.completion[-1])
