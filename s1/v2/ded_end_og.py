...
async def main():
    ...
    await baseline(base_endpoint, tokenizer, "logs/base_qwen25i_og", nrows) # baseline original model
    os.system("say 'done with original model baseline'")
    # await forcing(2, base_endpoint, tokenizer, "logs/force2_qwen25i_og", nrows) # 2k forcing original model
    # os.system("say 'done with original model 2k forcing'")
    # await forcing(4, base_endpoint, tokenizer, "logs/force4_qwen25i_og", nrows) # 4k forcing original model
    # os.system("say 'done with original model 4k forcing'")
    # await forcing(8, base_endpoint, tokenizer, "logs/force8_qwen25i_og", nrows) # 8k forcing original model
    # os.system("say 'done with original model 8k forcing'")

    # await baseline(ft_endpoint, tokenizer, "logs/base_qwen25i_ft", nrows) # baseline finetuned model
    # os.system("say 'done with finetuned model baseline'")
    # await forcing(2, ft_endpoint, tokenizer, "logs/force2_qwen25i_ft", nrows) # 2k forcing finetuned model
    # os.system("say 'done with finetuned model 2k forcing'")
    # await forcing(4, ft_endpoint, tokenizer, "logs/force4_qwen25i_ft", nrows) # 4k forcing finetuned model
    # os.system("say 'done with original model 4k forcing'")
    # await forcing(8, ft_endpoint, tokenizer, "logs/force8_qwen25i_ft", nrows) # 8k forcing finetuned model
